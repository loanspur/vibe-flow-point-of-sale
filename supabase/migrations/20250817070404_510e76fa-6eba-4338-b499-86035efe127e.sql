...
-- Update quotes table to ensure tenant_id is properly managed
-- First update any NULL tenant_id values to a valid tenant
UPDATE quotes 
SET tenant_id = (
    SELECT tenant_id 
    FROM profiles 
    WHERE user_id = quotes.cashier_id 
    LIMIT 1
) 
WHERE tenant_id IS NULL AND EXISTS (
    SELECT 1 FROM profiles WHERE user_id = quotes.cashier_id
);

-- Now set tenant_id as NOT NULL
ALTER TABLE quotes ALTER COLUMN tenant_id SET NOT NULL;

-- Drop existing policies if they exist to avoid conflicts
DROP POLICY IF EXISTS "Tenant users can view quote items" ON quote_items;
DROP POLICY IF EXISTS "Tenant staff can manage quote items" ON quote_items;
DROP POLICY IF EXISTS "Tenant users can view invoice payments" ON invoice_payments;
DROP POLICY IF EXISTS "Tenant staff can manage invoice payments" ON invoice_payments;
...
CREATE POLICY "Tenant users can view invoice payments" 
ON invoice_payments FOR SELECT 
USING (tenant_id = get_user_tenant_id());

CREATE POLICY "Tenant staff can manage invoice payments" 
