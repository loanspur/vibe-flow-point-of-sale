-- Fix database security warnings by dropping and recreating functions with proper search paths

-- Drop functions that need to be recreated with proper signatures
DROP FUNCTION IF EXISTS public.get_permission_error_message(text, text, text);
DROP FUNCTION IF EXISTS public.check_user_permission_enhanced(uuid, text, text);

-- Recreate get_permission_error_message function with secure search path
...
-- Recreate check_user_permission_enhanced function with secure search path
CREATE OR REPLACE FUNCTION public.check_user_permission_enhanced(
  p_user_id uuid, 
  p_resource text, 
...
-- Ensure get_current_user_role has proper search path
