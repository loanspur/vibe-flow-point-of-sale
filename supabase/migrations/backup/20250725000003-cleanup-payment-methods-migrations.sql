-- Cleanup redundant payment_methods migrations and ensure consistent schema
-- This migration consolidates all payment_methods table definitions into a single, consistent schema

-- First, check if payment_methods table exists, if not create it
DO $$ 
BEGIN
...
ALTER TABLE public.payment_methods ADD COLUMN IF NOT EXISTS processing_fee_percentage NUMERIC(5,2) DEFAULT 0.00;
ALTER TABLE public.payment_methods ADD COLUMN IF NOT EXISTS minimum_amount NUMERIC(15,2) DEFAULT 0.00;
ALTER TABLE public.payment_methods ADD COLUMN IF NOT EXISTS maximum_amount NUMERIC(15,2);
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'fk_payment_methods_tenant' 
    ...
    -- Add account foreign key if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
...
-- Insert default payment methods for existing tenants (only if they don't exist)
...
FROM tenants t
WHERE NOT EXISTS (
    SELECT 1 FROM payment_methods pm WHERE pm.tenant_id = t.id AND pm.name = 'Bank Transfer'
);
...
COMMENT ON COLUMN public.payment_methods.requires_reference IS 'Whether this payment method requires a reference number';
