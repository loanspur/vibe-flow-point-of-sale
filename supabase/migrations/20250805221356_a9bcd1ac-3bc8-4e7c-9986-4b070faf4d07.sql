...
DROP FUNCTION IF EXISTS public.calculate_next_billing_date(date);
DROP FUNCTION IF EXISTS public.calculate_prorated_amount(numeric, date, integer);
...
BEGIN
  ...
  ELSE
    ...
    -- Always bill on 1st of next month
    next_billing := DATE_TRUNC('month', start_date_param) + INTERVAL '1 month';
...
-- Create the calculate_next_billing_date function
CREATE FUNCTION public.calculate_next_billing_date(input_date date DEFAULT CURRENT_DATE)
RETURNS date
LANGUAGE plpgsql
IMMUTABLE
AS $$
...
-- Create calculate_prorated_amount function  
CREATE FUNCTION public.calculate_prorated_amount(
  full_amount numeric, 
  start_date date, 
