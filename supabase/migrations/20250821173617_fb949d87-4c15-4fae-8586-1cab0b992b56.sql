-- Fix the ambiguous user_id column issue in get_tenant_users_with_roles function
...
BEGIN
  ...
  -- Main query with explicit table aliases to avoid ambiguous column references
  RETURN QUERY
  SELECT 
    u.id AS user_id,
    u.email,
