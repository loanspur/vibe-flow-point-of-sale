-- Fix role management to support both complex role system and simple roles (tenant admin)
...
BEGIN
  ...
  -- Authorization - explicitly qualify user_id column
  SELECT get_current_user_role() INTO caller_role;
  SELECT EXISTS (
    SELECT 1 FROM tenant_users 
    WHERE tenant_users.tenant_id = effective_tenant_id 
  ...
  -- Main query with fallback to simple role system
  RETURN QUERY
  SELECT 
    u.id AS user_id,
    u.email,
    p.full_name,
    -- Primary role: Try complex role system first, fallback to simple roles
    COALESCE(
      (
        SELECT ur2.name
    ...
    -- Role names: Try complex role system first, fallback to simple roles
    COALESCE(
      (
  ...
  WHERE tu.tenant_id = effective_tenant_id
    ...
    AND (
      ...
      -- Check both complex and simple role systems
      (
        EXISTS (
        ...
        COALESCE(tu.role, p.role::text, 'user') ILIKE ('%' || p_role_filter || '%')
      )
    )
