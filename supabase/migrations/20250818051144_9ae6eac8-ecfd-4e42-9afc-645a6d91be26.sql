...
-- and add admin roles to all existing tenants
...
BEGIN
  ...
  ELSE
    -- If no owner found yet, we'll create the role later when the first admin is added
    -- This is handled by the handle_new_tenant_admin trigger
    NULL;
  END IF;
  ...
  LOOP
    ...
    IF existing_admin_role_id IS NULL THEN
      ...
      IF tenant_owner_id IS NOT NULL THEN
        ...
        -- Assign the Administrator role to the tenant owner
